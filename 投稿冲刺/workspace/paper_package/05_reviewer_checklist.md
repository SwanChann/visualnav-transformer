...
- [x] Table III comes from a clean, non-saturated, seeded closed-loop rerun with full
...
- [x] Abstract and contributions contain no simulation significance or real-robot
  outcome claim; conclusion must preserve the same boundary.
...
3. Are simulation comparisons confounded by stabilizer or duplicated trajectories?
   **The controlled run separates stabilizer strata, stores raw trajectories and
   excludes all historical records. Stabilizer-on trajectories are byte-identical
   across methods for each scene/seed, so those rows are disclosed as one overridden
   controller behavior with different compute costs, not independent method results.**
...
The package remains suitable for drafting, not submission. The controlled-simulation
gate is now complete, but repeated protocolized robot outcomes, target-device
deadline evidence and privacy/consent review remain hard blockers. Novelty is
moderate rather than strong; position the work as deployment characterization.
