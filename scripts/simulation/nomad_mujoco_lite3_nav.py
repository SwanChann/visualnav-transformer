...
════════════════════════════════════════════════════════════
  ...
  ▸ Step 1: 生成仿真 Topomap（固定地图导航前必须先生成对应地图的 topomap）
    ...
    python scripts/simulation/nomad_mujoco_lite3_nav.py --mode generate-topomap --map hard --topomap-nodes 30
  ...
  ▸ Step 2a: 固定地图导航（NoMaD + RL 联合仿真，到达目标返回 exit 0）
    ...
    python scripts/simulation/nomad_mujoco_lite3_nav.py --mode navigate --map hard --max-steps 200

  ▸ Step 2b: DDIM 加速推理（替代 DDPM，步数可选 10/5/3/2/1）
    python scripts/simulation/nomad_mujoco_lite3_nav.py --mode navigate --map easy --scheduler ddim
    python scripts/simulation/nomad_mujoco_lite3_nav.py --mode navigate --map easy --scheduler ddim --ddim-steps 5
    python scripts/simulation/nomad_mujoco_lite3_nav.py --mode navigate --map hard --scheduler ddim --ddim-steps 3

  ▸ Step 2c: 随机出生/目标点模式（自动生成在线 topomap，无需预生成）
    python scripts/simulation/nomad_mujoco_lite3_nav.py --mode navigate --map easy --random
    python scripts/simulation/nomad_mujoco_lite3_nav.py --mode navigate --map medium --random --scheduler ddim
    python scripts/simulation/nomad_mujoco_lite3_nav.py --mode navigate --map hard --random --no-gui

  ▸ 探索模式（NoMaD 无目标随机探索 + RL）
    python scripts/simulation/nomad_mujoco_lite3_nav.py --mode explore --map medium
    python scripts/simulation/nomad_mujoco_lite3_nav.py --mode explore --no-gui --max-steps 100
...
# ── 项目路径 ──────────────────────────────────────────────
from tooling.project_paths import REPO_ROOT, LITE3_LOCOMOTION_POLICY, LITE3_MJCF_XML
...
LITE3_XML = str(LITE3_MJCF_XML)
ONNX_POLICY = str(LITE3_LOCOMOTION_POLICY)
...
def write_topomap_metadata(topo_dir, domain, map_name=None, source=None):
    payload = {
        "domain": domain,
        "map": map_name,
        "generated_by": source or "scripts/simulation/nomad_mujoco_lite3_nav.py",
...
def run_generate_topomap(args):
    ...
    write_topomap_metadata(
        ...
        domain="mujoco",
        map_name=args.map,
        source="scripts/simulation/nomad_mujoco_lite3_nav.py --mode generate-topomap",
    )
    ...
    if args.mode == "walk-test":
        ...
        if args.random:
            ...
            if args.topomap_traj:
                ...
                print("  请改用: python scripts/simulation/nomad_mujoco_lite3_nav.py --mode generate-topomap --map "
                      f"{args.map}")
                sys.exit(1)
            # 自动检测 topomap 目录
