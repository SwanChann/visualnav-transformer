...
此外，$8 \times 8$ 的轨迹采样规格表示每次推理生成 8 条候选轨迹，每条轨迹包含 8 个 waypoint。候选轨迹的多样性为后续 TTS 搜索提供了选择空间，而 8 个 waypoint 的长度在 NoMaD 的默认配置下对应约 2--3 秒的未来规划窗口，这一时间尺度既足以覆盖局部导航决策（如是否转弯），又不至于因规划太远而积累过大误差。
...
从表中可以看出，TTS-8 与 TTS-16 均显著提升前进性并降低平滑性代价。TTS-16 的综合评分最高，但时延约为 TTS-8 的 2 倍；TTS-32 虽然进一步提高前进性，但时延增加到 26.50\,ms，综合评分反而下降。这说明 TTS 的收益存在边际递减，部署时更适合选择中等 budget，而不是简单增大搜索规模。
...
基于上述结果，可得到三点阶段性结论。第一，在不改动训练结果的前提下，\texttt{ddim\_2} 是当前最值得进入闭环验证的加速配置。第二，CFG 的有效范围有限，更适合作为可调参数而不是默认固定设置。第三，TTS-8/16 能够在可接受时延内提升候选轨迹质量，适合作为 DDIM 加速后的预算再分配方式。第四，视觉编码器升级已完成四个训练权重的离线统计，但最终部署选择仍需结合闭环验证与真机条件共同判断。
