...
值得注意的是，联合配置的 Top-5 中有三个为 $w=0.0$（即不开启 CFG），这进一步印证了 CFG 在当前任务中的边际收益有限。\texttt{ddim3\_w1.0} 和 \texttt{ddim3\_w0.5} 虽然进入了 Top-5，但其时延（约 27\,ms）已接近 \texttt{ddim5\_w0.0}（24.63\,ms），且综合评分并未显著超越后者，说明 CFG 引入的质量提升被时延代价部分抵消。

从推理层组合关系看，DDIM、CFG 与 TTS 可以统一收口在同一推理模块内，但不能简单看作并列开关。更合理的组合方式是：先由 DDIM 决定采样调度，再由 CFG 决定条件引导强度，最后由 TTS 在候选轨迹层进行搜索与筛选。这种层级关系可以用以下伪代码描述：DDIM 控制每条轨迹的生成速度，CFG 控制每条轨迹的生成质量偏好，TTS 在多条轨迹之间做最终决策。因此，本文建议后续闭环优先验证的主方案为"DDIM-2 + 可选 CFG + 可配置 TTS"，并以此作为第四章系统与仿真验证的输入。
