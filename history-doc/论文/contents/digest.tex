...
回答三个面向部署的问题：第一，NoMaD 的视觉编码器、距离预测头与扩散策略头在模型结构与推理流程上如何耦合，推理层有哪些优化空间；第二，是否可以在不重新训练模型的前提下，
通过采样加速、条件引导、测试时搜索与视觉编码器替换等手段显著提升策略可用性；
第三，这些优化能否整合进面向 Lite3 的分层闭环系统，并完成 MuJoCo 仿真与
Jetson Orin 真机部署的完整验证。技术路线沿``统一推理模块构建—系统层集成—部署层验证''
三级递进展开，对应论文第二、三、四章。

\subsection*{主要工作与创新点}

\textbf{统一推理模块}：建立了统一的离线统计实验链路，对 DDIM 推理加速、Classifier-Free
...
分层闭环系统，支持 MuJoCo 仿真与真机两种后端。状态机由空闲、站立、导航、
探索、恢复、急停、完成和失败等状态组成，通过统一接口与平台解耦；
导航主机支持以 JavaScript 对象表示法（JavaScript Object Notation, JSON）描述的批量计划和交互式任务两种模式，允许在同一会话内完成拍照、建图、
...
推理基准与端到端流水线；在桥接层对速度命令加入非法数值（Not a Number/infinity, NaN/Inf）守卫、幅值限幅与
连接健康监测，任意推理异常自动走安全急停；状态机层加入全局摔倒保护，使
空闲、站立等状态也不再遗漏摔倒判定。

\subsection*{实验与结论}

离线实验表明，DDIM-2 + 可选 CFG + 可配置 TTS 更适合进入后续闭环验证；
...
classical Simultaneous Localization and Mapping (SLAM)-plus-planner pipelines toward learning-based, image-conditioned policies
that consume raw visual observations directly.

Recently, general navigation models such as General Navigation Model (GNM), Visual Navigation Transformer (ViNT), and Goal Masked Diffusion Policies for Navigation and Exploration (NoMaD) have introduced
...
acceleration via Denoising Diffusion Implicit Model (DDIM), conditional guidance via Classifier-Free Guidance (CFG),
...
system for Lite3 and validated through Multi-Joint dynamics with Contact (MuJoCo) simulation as well as real-world
deployment on the Jetson Orin onboard computer. The technical roadmap proceeds through
unified-inference-module construction and optimization, system-level integration, and deployment-level
validation, corresponding respectively to Chapters 2, 3, and 4 of the thesis. A
...
\textbf{Unified inference module.} A unified offline statistical pipeline is built to compare
DDIM inference acceleration, CFG enhancement, Test-Time Scaling (TTS) search, and visual encoder upgrades under a single evaluation protocol.
Concretely, DDIM-2 attains about $4.84\times$ speedup over the Denoising Diffusion Probabilistic Model (DDPM)-10 baseline on the
...
To make the ranking criterion explicit, the configuration-level score used in the
TTS tables is defined as
$S_{\mathrm{rank}}=\overline{\mathrm{FP}}-0.050\,\overline{\mathrm{Latency}}
-0.350\,\overline{\mathrm{LA}}-0.150\,\overline{\mathrm{Smooth}}$,
where latency is measured in milliseconds, lateral deviation and smoothness are
penalized, and smoothness acts mainly as a weak regularizer because it is computed
from the mean second-order trajectory difference.
...
navigation-point-to-velocity mapping, and a platform interface---supports both MuJoCo and
...
high-level logic. The navigation host supports both batched JavaScript Object Notation (JSON) plans and interactive
single commands, allowing capture, topological map building, navigation, and exploration to be
composed within a single experiment session. Mid-level mapping converts the diffusion
policy's local navigation points into bounded forward, lateral, and yaw velocity commands,
...
experiments, Lite3 physically reaches the target in point-navigation and multi-goal
tasks under a $0.5\,\text{m}$ Euclidean-distance threshold, while the exploration mode
verifies that the goal-masked, target-free inference chain remains executable on the
same system. The same unified inference module that handles the four offline optimizations is
...
speed limits. The bridging layer wraps the Lite3 Software Development Kit (SDK) with a 25\,Hz velocity-refresh
...
Not a Number/infinity (NaN/Inf) clipping, magnitude limits, fallen-robot detection, and a connection-stale
...
topological maps record closed-loop frequencies of 2.89\,Hz (DDPM baseline), 6.80\,Hz
...
under the present conservative speed setting. Unlike MuJoCo, the real robot does not
use global ground-truth coordinates for success evaluation; success is determined by
a visual-topological arrival rule in which the selected topological node reaches the
goal node and the NoMaD distance head predicts a distance below the dimensionless
threshold $\tau_{\mathrm{arrive}}=3.0$, with no emergency stop or manual intervention.
The frequency ranking is fully
consistent with the offline analysis in Chapter 2: DDIM step count dominates latency,
...
hierarchical system can robustly organize quadruped navigation, multi-goal execution,
exploration, recovery, and emergency-stop transitions through one shared control
architecture. Stage-1 validation on Jetson Orin demonstrates the baseline
...
4\,cm---far smaller than the topological map node spacing---so high-level latency tolerance is
absorbed by the topological map sliding window and the 25\,Hz bridge replay rather than
...
the latency margin that the fastest scheme has reserved. Third, the topological map is still
